...
def basic_greedy_search(distances, start_city):
    # Initializes visited, path, current city, and the total distance
    visited = set()
    path = [start_city]
    current = start_city
    total_distance = 0
    while len(visited) < num_cities:
        # Adds the current city to visited
        visited.add(current)
        min_distance = float('inf')
        next_node = None
        # Iterates through all of the unvisited neighbours of the current city and selects the closest one
        ...
        if next_node is None:
            break
        # Adds the nearest neighbour to the path and adds the distance to the total distance
        path.append(next_node)
        total_distance += min_distance
        current = next_node
        # Returns the path and the distance from the current node to the end node added to the total distance
    return path, (total_distance + distances[start_city][current])


# Iterates through all possible starting nodes and outputs the best tour found
