...
# Same algorithm as the basic implementation but instead takes the argument of a path
def greedy_completion(distances, state):
    ...
    while len(visited) < num_cities:
        ...
        current = next_node
        # Outputs the lowest estimated greedy completion from the last node in the path
    return (total_distance + distances[start_city][current])

def basic_greedy_search(distances, start_city):
    visited = set()
    path = [start_city]
    current = start_city
    total_distance = 0
    while len(visited) < num_cities:
        ...
        next_node = None
        for neighbour in range(num_cities):
            # Creates a new path using the neighbour being considered and selects the neighbour with the lowest greedy completion cost as the next node
            state = path + [neighbour]
            h = greedy_completion(dist_matrix, state)
            if neighbour not in visited and (h < min):
                min = h
        ...
        current = next_node
    return path, (total_distance + distances[start_city][current])
...
if num_cities <= 58:
    for i in range(len(dist_matrix)):
        current_tour, current_distance = basic_greedy_search(dist_matrix, i)
    ...
    tour, tour_length = basic_greedy_search(dist_matrix, i)
